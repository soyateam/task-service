...
export class MongooseErrorHandler {
  ...
  private static readonly COLLECTION_NAMES = ['tasks'];
  private static readonly COLLECTION_NAMES_VIEW = ['Task'];
  ...
  private static parseDuplicateKeyMessage(message: string) {
    ...
    const prop = message.substring(
      collectionIndex + MongooseErrorHandler.COLLECTION_NAMES[index].length * 2 + 3,
      dupKeySepIndex - 3,
    );
